...
`oxify-engine` executes DAG-based API orchestration workflows. It resolves execution order via topological sort, executes independent nodes in parallel, and supports retry logic with exponential backoff.
...
```rust
use oxify_engine::rest_connector::{RestConfig, RestConnector, AuthConfig};
